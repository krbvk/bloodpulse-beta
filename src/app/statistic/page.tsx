...
function BloodTypeChart() {
  const mostNeeded = bloodTypeStats.reduce((a, b) => (b.needed > a.needed ? b : a));
  const mostDonated = bloodTypeStats.reduce((a, b) => (b.donated > a.donated ? b : a));
    ...
    <ResponsiveContainer width="100%" height={300}>
      <PieChart>
        <Pie
          ...
          label={({ type }) => type}
        >
          {neededBloodTypes.map((entry, index) => (
            <Cell
              key={`needed-pie-${index}`}
              fill={["#e03131", "#ffa8a8", "#fab005", "#845ef7", "#f59f00", "#1971c2"][index % 6]}
            />
          ))}
        </Pie>
        <Tooltip />
    ...
    <ResponsiveContainer width="100%" height={300}>
      <PieChart>
        <Pie
          ...
          label={({ type }) => type}
        >
          {donatedBloodTypes.map((entry, index) => (
            <Cell
              key={`donated-pie-${index}`}
              fill={["#1971c2", "#a5d8ff", "#fab005", "#5f3dc4", "#e03131", "#ffa8a8"][index % 6]}
            />
          ))}
        </Pie>
        <Tooltip />
...
const Page = () => {
  ...
  const { data: isUserDonor } = api.donor.getIsUserDonor.useQuery(undefined, {
                      ...
                      <Stack gap={6}>
                        {donorGender.map((g) => (
                          <Group key={g.label} gap="xs">
                            <Badge variant="filled" color={g.color as any} size="xs" radius="sm">
                              &nbsp;
                            </Badge>
                            <Text size="sm">
